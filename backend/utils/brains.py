...
def create_brain(commons: CommonsDep, brain=Brain):
    response = (
        commons["supabase"].table("brains").insert({"name": brain.name}).execute()
    )
    return response.data
...
def create_brain_vector(commons: CommonsDep, brain_id, vector_id):
    response = (
        commons["supabase"]
        .table("brains_users")
        .insert({"brain_id": brain_id, "vector_id": vector_id})
        .execute()
    )
    return response.data
...
def update_brain_fields(commons: CommonsDep, brain: Brain):
    # Need to only get the not undefined brain fields passed Optional['Brain'] -> create a Brain type
    commons["supabase"].table("brains").update({"name": brain.name}).match(
