...
def test_upload_explore_and_delete_file_video(client, api_key):
    # Retrieve the default brain
    brain_response = client.get(
        "/brains/default", headers={"Authorization": "Bearer " + api_key}
    )
    ...
    # File to upload
    file_path = "tests/test_files/test.mp4"
    file_name = "test.mp4"  # Assuming the name of the file on the server is the same as the local file name
