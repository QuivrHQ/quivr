...
def test_find_model_and_generate_metadata(mock_chat_service):
    ...
    user_settings = {"models": ["gpt-3.5-turbo"]}
    models_settings = [
        {"name": "gpt-3.5-turbo", "max_input": 512, "max_output": 512}
    ]
    ...
    assert model_to_use.name == "gpt-3.5-turbo"
...
def test_find_model_and_generate_metadata_user_not_allowed(mock_chat_service):
    ...
    assert isinstance(model_to_use, LLMModels)
    assert model_to_use.name == "gpt-3.5-turbo"  # Default model is used
