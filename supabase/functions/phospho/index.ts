...
serve(
  async (req: {
    json: () => ThreadHistoryPayload | PromiseLike<ThreadHistoryPayload>;
  }) => {
    if (!phosphoApiKey) {
      throw new Error("Missing Phospho API key");
    }

    const payload: ThreadHistoryPayload = await req.json();
    ...
    if (payload.record.user_message && payload.type === "UPDATE") {
        ...
        batched_log_events: [
          {
            ...
            session_id: payload.record.thread_id,
      ...
      if (!response.ok) {
        throw new Error(
          `Error sending thread data to Phospho: ${response.statusText}`
        );
      }
    ...
    return new Response("No new thread message detected", { status: 200 });
  }
);
