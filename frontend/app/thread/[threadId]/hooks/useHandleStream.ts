...
export const useHandleStream = () => {
  const { updateStreamingHistory } = useThreadContext();
    ...
    const handleStreamRecursively = async () => {
      ...
      if (done) {
        if (incompleteData !== "") {
          ...
          try {
            const parsedData = JSON.parse(incompleteData) as ThreadMessage;
      ...
      dataStrings.forEach((data, index, array) => {
        ...
        try {
          const parsedData = JSON.parse(data) as ThreadMessage;
          updateStreamingHistory(parsedData);
        } catch (e) {
