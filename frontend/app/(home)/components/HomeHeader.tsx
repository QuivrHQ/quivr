...
export const HomeHeader = (): JSX.Element => {
  ...
  const navLinks = navItems.map(({ href, label }) => (
    <li key={label}>
      <Link
        href={href}
        className="flex justify-between items-center hover:text-primary p-2 md:text-white md:hover:text-gray-300"
      >
        {label}
    ...
    <>
      <header className="w-screen flex justify-between items-center p-5 min-w-max">
        ...
        <div className="hidden md:flex">
          <ul className="flex gap-2 items-center">{navLinks}</ul>
        </div>
