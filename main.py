...
from explorer import view_document, view_summaries
...
if self_hosted == "false":
    ...
    if usage > st.secrets.usage_limit:
        ...
        st.markdown(
            f"<span style='color:blue'>Usage today: {usage} tokens out of {st.secrets.usage_limit}</span>", unsafe_allow_html=True)
    st.write("---")
...
if user_choice == 'Add Knowledge':
    ...
    st.info('Enable summarization will greatly improve information retrieval '
            'accuracy, but will use more token. (We use gpt-3.5-turbo '
            'for summarization)', icon="ℹ️")
    st.session_state['summarization'] = st.checkbox(
        'Summarize file content during adding knowledge')
    ...
    if self_hosted != "false":
        ...
        st.sidebar.write(
            "**Self Host to unlock more models such as claude-v1 and GPT4**")
        st.session_state['model'] = "gpt-3.5-turbo"
    ...
    chat_with_doc(vector_store, stats_db=supabase)
    ...
    st.sidebar.title("Configuration")
    st.info('**This feature is in active development**', icon="🛑")
    view_document(supabase)
    view_summaries(supabase)
